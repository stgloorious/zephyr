 ...
 * validates arguments.
 *
 * When CONFIG_LED_TRIGGER is enabled, led_on(), led_off(), and
 * led_set_brightness() cancel any active trigger on the channel before
 * dispatching to the driver, and led_blink() routes to the trigger
 * framework when the driver does not provide hardware blink.
 */
