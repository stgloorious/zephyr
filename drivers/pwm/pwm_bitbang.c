...
static void pwm_bitbang_timer_cb(const struct device *counter_dev, void *user_data)
{
	...
	for (uint8_t i = 0; i < cfg->num_channels; i++) {
		gpio_pin_set_dt(&cfg->gpios[i], data->tick < cfg->pulse[i] ? 1 : 0);
	}
			       ...
			       pwm_flags_t flags)
{
	...
	LOG_DBG("set_cycles: ch=%u period=%u pulse=%u", channel, period_cycles, pulse_cycles);
...
static int pwm_bitbang_init(const struct device *dev)
{
	...
	LOG_INF("init: dev=%s channels=%u", dev->name, cfg->num_channels);
	...
	LOG_INF("init: counter freq=%u Hz (= cycles/sec)", data->freq);
