...
struct led_trigger {
	 ...
	 *
	 * The trigger should initialise its per-channel state in place at
	 * @c &ch->data.<member> and prepare to drive the LED.
	 *
	 * Called with @c ch->lock held and @c ch->transition_lock serialising
	 ...
	 * @c &ch->data and quiesce before returning.
	 *
...
#endif /* CONFIG_LED_TRIGGER_TIMER */

/**
 * @brief Storage union large enough to hold any enabled trigger's
 *        per-channel state.
 *
 * Embedded directly into @ref led_trigger_channel: each channel can have
...
#ifdef CONFIG_LED_TRIGGER_TIMER
	struct led_trigger_timer_data timer;
#endif
	/* Keep the union non-empty if no triggers are enabled, and force
	 * pointer alignment.
	 */
	void *_placeholder;
};
 ...
 *
 * - @c transition_lock serialises trigger activate/deactivate so the two
...
struct led_trigger_channel {
	...
	struct k_mutex transition_lock;
